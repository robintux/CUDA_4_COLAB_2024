...
/* Kernel for matrix multiplication, w/o parallelization 
   -> every thread works on all matrix elements */
...
int main(int argc, char * argv[])
{
  ...
  if ( argc != 3 ) {
    cout << "Need two arguments: number of columns (= number of rows) of matrix and device to use" << endl;
    return -1;
  }
  ...
  printf("matrix size = %u", matrix_size);
